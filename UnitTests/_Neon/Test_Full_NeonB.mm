//
//  Test_Full_NeonB.mm
//  CyberMatrixTests
//

#import <XCTest/XCTest.h>

#include "M88.hpp"
#include "VerifyFull.hpp"
#include "Tests.hpp"
#include "Logging.hpp"

@interface Test_Full_NeonB : XCTestCase

@end

@implementation Test_Full_NeonB

- (void)testFull_NeonB_4x4 {
    M88 aBefore = VerifyFull::SpawnTestMatrixFull();
    M88 aAfter = aBefore;

    aAfter.Full_NeonB_4x4();

    const M aExpected = {
        {  4,  5,  6,  7, 54, 55, 52, 53 },
        { 12, 13, 14, 15, 62, 63, 60, 61 },
        { 20, 21, 22, 23, 38, 39, 36, 37 },
        { 28, 29, 30, 31, 46, 47, 44, 45 },
        { 18, 19, 16, 17, 32, 33, 34, 35 },
        { 26, 27, 24, 25, 40, 41, 42, 43 },
        {  2,  3,  0,  1, 48, 49, 50, 51 },
        { 10, 11,  8,  9, 56, 57, 58, 59 }
    };

    if (!VerifyFull::CheckFull(aBefore, aAfter, aExpected)) {
        Logging::Log("Before", aBefore);
        Logging::Log("After", aAfter);
        XCTFail(@"%s failed", sel_getName(_cmd));
        return;
    }
}

- (void)testFull_NeonB_EachQuad_4x4 {
    M88 aBefore = VerifyFull::SpawnTestMatrixFull();
    M88 aAfter = aBefore;

    aAfter.Full_NeonB_EachQuad_4x4();

    const M aExpected = {
        {  2,  3, 27, 26,  6,  7, 31, 30 },
        { 10, 11, 19, 18, 14, 15, 23, 22 },
        {  9,  8, 16, 17, 13, 12, 20, 21 },
        {  1,  0, 24, 25,  5,  4, 28, 29 },
        { 34, 35, 59, 58, 38, 39, 63, 62 },
        { 42, 43, 51, 50, 46, 47, 55, 54 },
        { 41, 40, 48, 49, 45, 44, 52, 53 },
        { 33, 32, 56, 57, 37, 36, 60, 61 }
    };

    if (!VerifyFull::CheckFull(aBefore, aAfter, aExpected)) {
        Logging::Log("Before", aBefore);
        Logging::Log("After", aAfter);
        XCTFail(@"%s failed", sel_getName(_cmd));
        return;
    }
}

- (void)testFull_NeonB_8x8 {
    M88 aBefore = VerifyFull::SpawnTestMatrixFull();
    M88 aAfter = aBefore;

    aAfter.Full_NeonB_8x8();

    const M aExpected = {
        {  5, 13,  7, 15, 55, 63, 53, 61 },
        {  4, 12,  6, 14, 54, 62, 52, 60 },
        { 21, 29, 23, 31, 39, 47, 37, 45 },
        { 20, 28, 22, 30, 38, 46, 36, 44 },
        { 19, 27, 17, 25, 33, 41, 35, 43 },
        { 18, 26, 16, 24, 32, 40, 34, 42 },
        {  3, 11,  1,  9, 49, 57, 51, 59 },
        {  2, 10,  0,  8, 48, 56, 50, 58 }
    };

    if (!VerifyFull::CheckFull(aBefore, aAfter, aExpected)) {
        Logging::Log("Before", aBefore);
        Logging::Log("After", aAfter);
        XCTFail(@"%s failed", sel_getName(_cmd));
        return;
    }
}

@end
