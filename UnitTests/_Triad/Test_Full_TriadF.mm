//
//  TestFullKIND.m
//  CyberMatrixTests
//

#import <XCTest/XCTest.h>

#include "M88.hpp"
#include "VerifyFull.hpp"
#include "Tests.hpp"
#include "Logging.hpp"

@interface Test_Full_TriadF : XCTestCase

@end

@implementation Test_Full_TriadF

- (void)testFull_TriadF_2x2 {
    M88 aBefore = VerifyFull::SpawnTestMatrixFull();
    M88 aAfter = aBefore;

    aAfter.Full_TriadF_2x2();

    const M aExpected = {
        { 36, 37, 38, 39,  4,  5,  6,  7 },
        { 44, 45, 46, 47, 12, 13, 14, 15 },
        { 52, 53, 54, 55, 20, 21, 22, 23 },
        { 60, 61, 62, 63, 28, 29, 30, 31 },
        {  0,  1,  2,  3, 32, 33, 34, 35 },
        {  8,  9, 10, 11, 40, 41, 42, 43 },
        { 16, 17, 18, 19, 48, 49, 50, 51 },
        { 24, 25, 26, 27, 56, 57, 58, 59 }
    };

    if (!VerifyFull::CheckFull(aBefore, aAfter, aExpected)) {
        Logging::Log("Before", aBefore);
        Logging::Log("After", aAfter);
        XCTFail(@"%s failed", sel_getName(_cmd));
        return;
    }
}

- (void)testFull_TriadF_EachQuad_2x2 {
    M88 aBefore = VerifyFull::SpawnTestMatrixFull();
    M88 aAfter = aBefore;

    aAfter.Full_TriadF_EachQuad_2x2();

    const M aExpected = {
        { 18, 19,  2,  3, 22, 23,  6,  7 },
        { 26, 27, 10, 11, 30, 31, 14, 15 },
        {  0,  1, 16, 17,  4,  5, 20, 21 },
        {  8,  9, 24, 25, 12, 13, 28, 29 },
        { 50, 51, 34, 35, 54, 55, 38, 39 },
        { 58, 59, 42, 43, 62, 63, 46, 47 },
        { 32, 33, 48, 49, 36, 37, 52, 53 },
        { 40, 41, 56, 57, 44, 45, 60, 61 }
    };

    if (!VerifyFull::CheckFull(aBefore, aAfter, aExpected)) {
        Logging::Log("Before", aBefore);
        Logging::Log("After", aAfter);
        XCTFail(@"%s failed", sel_getName(_cmd));
        return;
    }
}

- (void)testFull_TriadF_EachMini_2x2 {
    M88 aBefore = VerifyFull::SpawnTestMatrixFull();
    M88 aAfter = aBefore;

    aAfter.Full_TriadF_EachMini_2x2();

    const M aExpected = {
        {  9,  1, 11,  3, 13,  5, 15,  7 },
        {  0,  8,  2, 10,  4, 12,  6, 14 },
        { 25, 17, 27, 19, 29, 21, 31, 23 },
        { 16, 24, 18, 26, 20, 28, 22, 30 },
        { 41, 33, 43, 35, 45, 37, 47, 39 },
        { 32, 40, 34, 42, 36, 44, 38, 46 },
        { 57, 49, 59, 51, 61, 53, 63, 55 },
        { 48, 56, 50, 58, 52, 60, 54, 62 }
    };

    if (!VerifyFull::CheckFull(aBefore, aAfter, aExpected)) {
        Logging::Log("Before", aBefore);
        Logging::Log("After", aAfter);
        XCTFail(@"%s failed", sel_getName(_cmd));
        return;
    }
}

@end
